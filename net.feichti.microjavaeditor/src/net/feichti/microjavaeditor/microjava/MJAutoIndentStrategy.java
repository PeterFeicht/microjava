...
public class MJAutoIndentStrategy extends DefaultIndentLineAutoEditStrategy
{
	@Override
	 ...
	 */
	protected static int findMatchingOpenBracket(IDocument document, int line, int end, int closingBracketIncrease)
		...
		while(brackcount < 0) {
			...
			start = document.getLineOffset(l);
			e = start + document.getLineLength(l) - 1;
			brackcount += getBracketCount(document, start, e, false);
		}
		return l;
	}
	
	/**
	 * Returns the bracket value of a section of text. Closing brackets have a value of -1 and open brackets
	 * have a value of 1.
	 *
	 * @param document - the document being parsed
	...
	private static int getBracketCount(IDocument document, int start, int end, boolean ignoreCloseBrackets)
		...
		int begin = start;
		int bracketcount = 0;
		boolean ignoreClose = ignoreCloseBrackets;
	 ...
	 * @throws BadLocationException in case <code>position</code> and <code>end</code> are invalid in the
	 *             document
	 */
	 ...
	 */
	private static int getStringEnd(IDocument document, int position, int end, char character)
			throws BadLocationException {
		int currentPosition = position;
		while(currentPosition < end) {
	...
	protected void smartIndentAfterNewLine(IDocument document, DocumentCommand command) {
		...
		try {
			...
			System.out.println("MJAutoIndentStrategy.smartIndentAfterNewLine: BadLocationException");
		}
	}
	...
	protected void smartInsertAfterBracket(IDocument document, DocumentCommand command) {
		...
		try {
			...
			System.out.println("MJAutoIndentStrategy.smartInsertAfterBracket: BadLocationException");
		}
	}
}
